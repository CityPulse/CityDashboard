...
public class InterfaceBackendServices extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) {
		...
		//this path will get the unique sensorCategories and their list of fields
		if(servletPath.toUpperCase().equals("/firstStep".toUpperCase()))
		{
			...
			for (String category : resultSetSensors.keySet()) {
				...
				if(localStreamDescription!=null)
				{
					...
					// I only need the fields that have the ShowOnCityDashboard equal to true
		...
		//this path will get all the sensors coordinates of that type and also the last value that they had
		else if (servletPath.toUpperCase().equals("/secondStep".toUpperCase())) 
		{
			...
			for(String uuid : listOfUUIDs)
			{
				...
				if(streamDescription!=null)
				{
					...
					String lastValue = getLastValueOfSensor(uuid,field);
		...
		//path used to get historic data of a particular sensor and field in a specific time interval
		else if(servletPath.toUpperCase().equals("/thirdStep".toUpperCase())) 
		{
			ArrayList<String> listOfValues = new ArrayList<String>();
			ArrayList<String> listOfDates = new ArrayList<String>();
			
			String url = "http://131.227.92.55:8017/api/snapshot_sql?uuid=UUID_OF_SENSOR&start=START_DATE&end=END_DATE&format=json";
			String uuid, dateStart, dateEnd, field;
			...
			if((field.equals("SO2"))||(field.equals("O3"))||(field.equals("PM10"))||(field.equals("NO2"))||(field.equals("CO")))
			{
	...
	private String getLastValueOfSensor(String uuid, String field) {
		...
		String url = "http://131.227.92.55:8017/api/snapshot_sql?uuid=UUID_OF_SENSOR&start=START_DATE&end=END_DATE&format=json&last=true&fields=FIELD_NAME";
	    ...
	    JSONObject nestedJson = obj.getJSONObject(field);
	    value = nestedJson.optString("value", "ERROR GETTING VALUE");
	...
	public static String getResponseFromURL(String url) {
		...
		try {
			...
			//logger.error("Could not connect to URL: "+url+" because it was malformed: " +e1);
		}
